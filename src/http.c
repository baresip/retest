...
#define REQ_BODY_CHUNK_SIZE (26 * 42)
#define REQ_BODY_SIZE (REQ_BODY_CHUNK_SIZE * 10 - 26)
#define NUM_HTTP_REQUESTS 5
...
static size_t http_req_body_handler(uint8_t **buf, void *arg)
{
	struct test *t = arg;
	size_t l = 0;
	size_t wlen;

	*buf = mem_alloc(REQ_BODY_CHUNK_SIZE, NULL);
	if (!*buf)
		return -1;
	...
	while ( l < REQ_BODY_CHUNK_SIZE && t->i_req_body < t->clen) {
		...
		memcpy(*buf + l, "abcdefghijklmnopqrstuvwxyz", wlen);
		l += wlen;
		t->i_req_body += wlen;
	}
...
static int test_http_loop_base(bool secure, const char *met)
{
	...
	for (i = 1; i <= NUM_HTTP_REQUESTS; i++) {
		...
		err = re_main_timeout((secure || put) ? 1800 : 900);
		if (err)
			goto out;
